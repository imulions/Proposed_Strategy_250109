function [x xd t xT x_obs]=SimulationUnderDisR1(x0,xT,fn_handle,v_exp,x_exp,DisData,varargin)
%
...
% systems:
%                               xd=f(x)
% 此函数模拟使用 SEDS 学习的运动，它定义了
% a motins 作为非线性与时间无关的渐近稳定动力学系统：
%               xd=f(x)
%
% where x is an arbitrary d dimensional variable, and xd is its first time
% derivative.
% 其中 x 是任意 d 维变量，xd 是其一阶时间导数。
% 
...
%% 一个句柄函数，仅获取 d x N 矩阵作为输入，并返回相同维度的输出矩阵。
%% 请注意，输出变量是输入变量的一阶导数。
...
while true
    ...
    if i >= size(x_exp, 2)
        ...
        if size(t, 2) > 20 && size(t, 2) < 120
            Dis = DisData(:,num,:);
            x(:,i+1)=x(:,i)+xd(:,i)*options.dt + [Dis(2,1); Dis(2,2)]/8*50;
